...
- \`clear\`: delete the cache entry`,
      ...
      exam: [
        ...
        { question: "How do you pass sensitive data (API keys, passwords) to GitLab CI jobs without storing them in .gitlab-ci.yml?", answer: "Use GitLab CI/CD Variables: Settings → CI/CD → Variables. These are injected as environment variables in every job. Mark sensitive variables as 'Masked' (redacted from job logs) and 'Protected' (only available in protected branches/tags). Reference in jobs: $MY_SECRET or ${MY_SECRET}. For group-wide secrets, use Group-level Variables (Settings → CI/CD → Variables at the group level). For secrets that rotate or need audit trails, use an external vault: integrate with HashiCorp Vault via the CI_JOB_JWT token or use GitLab's built-in Vault integration (Settings → CI/CD → Secrets). Never commit secrets in .gitlab-ci.yml even in encrypted form.", difficulty: "junior" },
      ],
    },
    {
